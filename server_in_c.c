...
//                 client_socket_and_address *thread_args = malloc(sizeof(client_socket_and_address));
//                 *thread_args = (client_socket_and_address){client_socket, their_addr};
...
// // maybe add a time stamp, be carful with the threads!! like in the job interview. using time_t and time(NULL)
// void signalHandler(int signal) {
//         char msg1[MSGSIZE];
//         if (signal == SIGPIPE){
//         strcpy(msg1, "Client disconnected unexpectedly.\n");
//         // Handle broken pipe error (code=141), unexpected client disconnected.
//         }
//         int file = open("log.txt", O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR);
